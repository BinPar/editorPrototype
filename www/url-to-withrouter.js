function addWithRouterImport(j, root) {
  ...
  if (originalRouterImport.length > 0) {
    ...
    if (
      originalRouterImport.find(j.ImportSpecifier, { imported: { name: 'withRouter' } }).length > 0
    ) {
      return;
    }
...
// For example if `functionName` is `withRouter` it will wrap the provided node in
// `withRouter(NODE_CONTENT)`
function wrapNodeInFunction(j, functionName, args) {
  const mappedArgs = args.map((node) => {
    ...
    if (reference.type === 'ClassDeclaration') {
      reference.type = 'ClassExpression';
    }
